...
path1.each do |move|
    ...
    dist = move[1..-1].to_i
    if dir == "R"
        (x1...(x1+dist)).each do |val|
            point1.push({ y: y1, x: val })
        end
        x1 += dist
    elsif dir == "D"
        (y1...(y1+dist)).each do |val|
            point1.push({ y: val, x: x1 })
        end
        y1 += dist
    elsif dir == "L"
        x1.downto(x1-dist + 1).each do |val|
            point1.push({ y: y1, x: val })
        end
        x1 -= dist
    elsif dir == "U"
        y1.downto(y1-dist + 1).each do |val|
            point1.push({ y: val, x: x1 })
        end
        y1 -= dist
    else
...
path2.each do |move|
    ...
    dist = move[1..-1].to_i
    if dir == "R"
        (x2...(x2+dist)).each do |val|
            point2.push({ y: y2, x: val })
        end
        x2 += dist
    elsif dir == "D"
        (y2...(y2+dist)).each do |val|
            point2.push({ y: val, x: x2 })
        end
        y2 += dist
    elsif dir == "L"
        x2.downto(x2-dist + 1).each do |val|
            point2.push({ y: y2, x: val })
        end
        x2 -= dist
    elsif dir == "U"
        y2.downto(y2-dist + 1).each do |val|
            point2.push({ y: val, x: x2 })
        end
        y2 -= dist
    else
...
intersections = point1.to_a & point2.to_a
dist = intersections.map do |h|
    [h[:y].abs + h[:x].abs, h]
end
